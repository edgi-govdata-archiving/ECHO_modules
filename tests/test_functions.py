...
county, state = get_spatial_data( region_type = "County", states = ["NY"], region_filter = "Erie")
...
show_regions(regions = watersheds, states = state, region_type = "Watershed", spatial_tables = spatial_tables)
...
"""
## EJSCREEN
EJSCREEN was a USEPA tool for sharing environmental and socioeconomic data relevant to environmental justice. In February 2025, USEPA removed its EJSCREEN website from public access, including an API for querying EJSCREEN indices/indicators and Census data. 

The [EJAM-API](https://github.com/edgi-govdata-archiving/EJAM-API) was built to restore programmatic access to EJSCREEN data. 

The `get_ejscreen()` function provides a wrapper for the API.

As an example, the following retrieves the active facilities for Erie County, NY, filters them to CAA-regulated facilities, retrieves EJSCREEN info for 1 mile radius areas around each facility, and then maps the "state.EJ.DISPARITY.traffic.score.supp" metric for each facility neighborhood.
"""

from ECHO_modules.utilities import get_active_facilities # Use the get_active_facilities function

erie = get_active_facilities("NY", "County", ["ERIE"]) # Get active facilities in Erie Conty

erie_air = erie[erie["AIR_FLAG"]=="Y"] # Filter to CAA facilities

from ECHO_modules.get_data import get_ejscreen # Import the get_ejscreen function
erie_air_gdf = get_ejscreen(regions=erie_air, region_type="sites", geometries=True, buffer=1) # The regions are each Erie County CAA facility, which are "sites" with lat/lng coordinates and we want info for 1 mile radius neighborhoods around each site. We want to return the lat/lng coordinates in the output (geometries=True)

from ECHO_modules.utilities import choropleth # Import the choropleth function
